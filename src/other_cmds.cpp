...
void Server::invitecmd(Message *msg, Client *cl)
{
	...
	if ((msg->params[1][0] == '#' || msg->params[1][0] == '&') &&
	    _m_chans.find(msg->params[1]) != _m_chans.end()) {
		for (std::vector<Client *>::iterator it =
			     _m_chans[msg->params[1]].begin();
		     it != _m_chans[msg->params[1]].end(); it++) {
		...
		send(_m_nickdb[nickname].top()->clfd, to_send.c_str(),
		     to_send.length(), 0);
		to_send = ":" + _m_nickdb[nickname].top()->prefix + " 341 " +
			  cl->nickname + " " + nickname + " " + msg->params[1] +
			  "\r\n";
...
void Server::topiccmd(Message *msg, Client *cl)
{
	if (msg->params.size() == 0) {
		...
		if (_m_chans.find(chan) == _m_chans.end())
			...
			send_to_channel(":" + cl->prefix + " TOPIC " + chan +
						" :" + msg->params[1] + "\r\n",
					chan, NULL);
		}
	}
...
void Server::kickcmd(Message *msg, Client *cl)
{
	...
	if (_m_chans.find(chan) == _m_chans.end())
		...
		if (msg->params.size() >= 3)
			...
			comment = cl->nickname;
		send_to_channel(":" + cl->prefix + " KICK " + chan + " " +
					user + " :" + comment + "\r\n",
				chan, NULL);
